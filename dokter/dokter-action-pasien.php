...
if (isset($_GET['logout'])) {
  	session_destroy();
  	unset($_SESSION['dokter_username']);
  	header("location: dokter-login.php");
}

if(isset($_POST["dokter-action"])) {
    // username and password sent from form

    $id_periksa = $_GET['id_periksa'];
    
    $sql_periksa = "SELECT * from periksa where id_periksa = '$id_periksa'";
    $periksa_result = mysqli_query($conn, $sql_periksa);
    $sql_dokter = "SELECT * from tb_doctors where username = '{$_SESSION['dokter_username']}'";
    $dokter_result = mysqli_query($conn, $sql_dokter);
    // $sql_keluhan = "SELECT * from periksa where id_periksa = '$id_periksa'";
    // $keluhan_result = mysqli_query($conn, $sql_keluhan);

    // $name = mysqli_real_escape_string($fetch['name']);
    $periksa = mysqli_fetch_assoc($periksa_result);
    $dokter = mysqli_fetch_assoc($dokter_result);
    // $keluhan = mysqli_fetch_assoc($keluhan_result);
    $diagnosa = mysqli_real_escape_string($conn, $_POST['diagnosa']);
    $tindakan = mysqli_real_escape_string($conn, $_POST['tindakan']);
    $tgl_berobat = date('Y-m-d', strtotime($_POST['tgl_berobat']));
    $catatan = mysqli_real_escape_string($conn, $_POST['catatan']);


    if (empty($tgl_berobat)) {
      array_push($errors, "Tanggal Berobat is required");
    }
    if (empty($tindakan)) {
      array_push($errors, "tindakan is required");
    }
    if (empty($diagnosa)) {
      array_push($errors, "diagnosa is required");
    }

    
    if (count($errors) == 0) {
        $sql = "SELECT * FROM hasil_pemeriksaan_dokter WHERE id_periksa='$id_periksa' AND username_pasien='{$periksa['username']}'";
        $results = mysqli_query($conn, $sql);
        if (mysqli_num_rows($results) == 1) {
            echo "<script>alert('Data Sudah Terdaftar di Sistem!'); window.location = 'dokter-check-pasien.php';</script>";
        }else {
            $sql = "INSERT INTO hasil_pemeriksaan_dokter (id_periksa, username_pasien, dokter, keluhan, diagnosa, tindakan, tgl_berobat) VALUES ('$id_periksa', '{$periksa['username']}', '{$dokter['name']}', '{$periksa['keluhan']}', '$diagnosa', '$tindakan', '$tgl_berobat')";
            //   $sql = "INSERT INTO periksa (id_periksa, username_pasien, dokter, keluhan_pasien, diagnosa_dokter, tindakan, tgl_berobat) VALUES ('$id_periksa', '{$username['username']}', '{$dokter['name']}', '{$keluhan['keluhan']}', '$diagnosa', '$tindakan', '$tgl_berobat')";
            //   $sql = "INSERT INTO rekam_medis (diagnosa_dokter, tindakan, tgl_berobat) VALUES ('$diagnosa', '$tindakan', '$tgl_berobat') WHERE id_periksa = '$id_periksa'";
              mysqli_query($conn, $sql);
              echo "<script>alert('Berhasil menyimpan data'); window.location = 'dokter-check-pasien.php';</script>";
        }
    }
 }
      ...
      <section>
        ...
        <section>
          <div class="d-flex justify-content-between align-items-center gap-3">
            <h4 class="title-section-content">Aksi Dokter</h4>
          </div>
              ...
              <div class="card">
                  <div class="p-4">
                      ...
                      <?php include '../errors.php'; ?>
                          ...
                          <button type="submit" class="btn btn-primary btn-block" name="dokter-action">Save</button>
                      </form>
                  </div>
              </div>
          </div>
